...
function leaseCountdown(leaseEnd: string | null): { text: string; color: string } {
  if (!leaseEnd) return { text: "No end date", color: "rgba(237,232,225,0.42)" };
  ...
  if (days === 0) return { text: "Expires today", color: "#fbbf24" };
  if (days <= 60) return { text: `${days} days left`, color: "#fbbf24" };
...
export function TenantCard({ tenant, rentHistory, index }: Props) {
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div
            style={{
              ...
              height: "36px",
              borderRadius: "50%",
              background: "rgba(139,32,48,0.18)",
              display: "flex",
              alignItems: "center",
          ...
          <div>
            <p style={{ color: "#EDE8E1", fontWeight: 600, fontSize: "14px", fontFamily: "var(--font-outfit)" }}>
              {tenant.name}
            </p>
            <p style={{ color: "rgba(237,232,225,0.42)", fontSize: "12px" }}>
              {tenant.status}
            </p>
          </div>
        </div>
      ...
      >
        <div>
          <p style={{ color: "rgba(237,232,225,0.20)", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "2px" }}>
            Lease Period
          </p>
          <p style={{ color: "rgba(237,232,225,0.42)", fontSize: "12px" }}>
            {tenant.leaseStart
      ...
      {last6.length > 0 && (
        <div>
          <p style={{ color: "rgba(237,232,225,0.20)", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "6px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
            {last6.map((r, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px" }}>
                ...
                <span style={{ color: "rgba(237,232,225,0.20)", fontSize: "9px" }}>
                  {r.month.slice(0, 1)}
                </span>
              </div>
            ))}
