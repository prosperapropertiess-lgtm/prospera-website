...
export async function generateStaticParams() {
  return getNeighbourhoodsByCity("st-thomas").map((n) => ({ neighbourhood: n.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { neighbourhood } = await params;
  const n = getNeighbourhood("st-thomas", neighbourhood);
  if (!n) return {};
  return {
    title: `Property Management in ${n.name}, St. Thomas ON`,
    description: `Professional property management in ${n.name}, St. Thomas ON. ${n.description} Contact Prospera Properties for a free quote.`,
  };
}

export default async function StThomasNeighbourhoodPage({ params }: Props) {
  const { neighbourhood } = await params;
  const n = getNeighbourhood("st-thomas", neighbourhood);
  if (!n) notFound();
  ...
  const schema = {
    ...
    "@type": "LocalBusiness",
    "@id": `https://www.prosperaproperties.co/areas/st-thomas/${n.slug}`,
    name: "Prospera Properties",
    description: `Property management in ${n.name}, St. Thomas, Ontario`,
    ...
    areaServed: { "@type": "Place", name: `${n.name}, St. Thomas, Ontario` },
    ...
    <div style={{ backgroundColor: "#F7F5F2" }} className="min-h-screen">
      ...
      <section className="py-20 px-6 text-center" style={{ backgroundColor: "#1F2F3A" }}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href="/contact" className="inline-block px-8 py-3 text-xs uppercase tracking-widest btn-primary rounded" style={{ backgroundColor: "#8B2030", color: "#FAF8F5", fontFamily: "var(--font-dm-sans)" }}>
            Get a Free Quote
          </Link>
          <Link href="/listings" className="inline-block px-8 py-3 text-xs uppercase tracking-widest border btn-primary rounded" style={{ borderColor: "rgba(250,248,245,0.3)", color: "#FAF8F5", fontFamily: "var(--font-dm-sans)" }}>
