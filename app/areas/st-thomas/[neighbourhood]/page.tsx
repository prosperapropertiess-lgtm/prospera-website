...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!n) return {};
  return {
    title: `Property Management in ${n.name}, St. Thomas Ontario`,
...
export default async function StThomasNeighbourhoodPage({ params }: Props) {
  ...
  const schema = {
    ...
    url: `https://www.prosperaproperties.co/areas/st-thomas/${n.slug}`,
    ...
    <div style={{ backgroundColor: "#0A1628" }} className="min-h-screen">
      ...
      <div className="pt-28 pb-2 px-6 max-w-5xl mx-auto">
        <p className="text-xs" style={{ color: "#5A7090", fontFamily: "var(--font-dm-sans)" }}>
          <Link href="/areas/st-thomas" className="hover:underline" style={{ color: "#C5A55A" }}>St. Thomas</Link>
          {" → "}
          <span>{n.name}</span>
        </p>
      </div>
      ...
      <section className="py-20 px-6 text-center" style={{ backgroundColor: "#060E1C" }}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href="/contact" className="inline-block px-8 py-3 text-xs uppercase tracking-widest transition-opacity hover:opacity-80" style={{ backgroundColor: "#C5A55A", color: "#FAF8F5", fontFamily: "var(--font-dm-sans)" }}>
            Get a Free Quote
          </Link>
